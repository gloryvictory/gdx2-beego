...
func (t *Stp) TableName() string {
	return "stp"
}

func init() {
	orm.RegisterModel(new(Stp))
}

// AddStp insert a new Stp into database and returns
// last inserted Id on success.
func AddStp(m *Stp) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetStpById retrieves Stp by Id. Returns error if
// Id doesn't exist
func GetStpById(id int) (v *Stp, err error) {
	o := orm.NewOrm()
	v = &Stp{Id: id}
...
// GetAllStp retrieves all Stp matches certain condition. Returns empty list if
// no records exist
func GetAllStp(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Stp))
	// query k=v
	for k, v := range query {
